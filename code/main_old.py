...
# create a dictionary where all of the classes that a teacher teaches are stored
professor_courses = {}
...
def generate_genome():
    ...
    for etapa in range (1, 9):
        if etapa < 4:
            size_even += len(class_groups) * len(dataset_courseSchedule_semester_even[dataset_courseSchedule_semester_even['ET'] == etapa])
            size_uneven += len(class_groups) * len(dataset_courseSchedule_semester_uneven[dataset_courseSchedule_semester_uneven['ET'] == etapa])
        else:
            size_even += len(dataset_courseSchedule_semester_even[dataset_courseSchedule_semester_even['ET'] == etapa])
            size_uneven += len(dataset_courseSchedule_semester_uneven[dataset_courseSchedule_semester_uneven['ET'] == etapa])
...
def fitness(genome):
    ...
    print("score_100_rule: ", score_100_rule)
    print("score_schedule_at: ", score_schedule_at)
    print("score_emergency_saturday_class: ", score_emergency_saturday_class)
    print("score_class_scheduling_count: ", score_class_scheduling_count)
    print("score_professor_scheduling: ", score_professor_scheduling)
...
def run_genetic_algorithm(generation_limit, fitness_limit):
    ...
    # analyze a random solution of the population
    translation = translate_genome(population[0])
